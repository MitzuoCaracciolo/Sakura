...
namespace Sakura
{
	...
	{
		return std::make_shared<DirectXIndexBuffer>(spec, std::dynamic_pointer_cast<DirectXContext>(context));
	}
		...
		: m_Spec(spec), m_Context(context)
	{
		...
		IndexBufferDesc.ByteWidth = sizeof(uint32_t) * m_Spec.Count;
	...
	{
		m_Context->m_DeviceContext->IASetIndexBuffer(m_Buffer.Get(), DXGI_FORMAT_R32_UINT, 0);
	}
